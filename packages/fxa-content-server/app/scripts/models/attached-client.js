...
var AttachedClient = Backbone.Model.extend({
  ...
  initialize (attrs, options) {
    ...
    // a single stable unique identifier. It's only for our internal use
    // by backbone's collection infrastructure.
    this.set('id', `${attrs.clientId || ''}-${attrs.deviceId || ''}-${attrs.refreshTokenId || ''}-${attrs.sessionTokenId || ''}`);
  ...
  // remove from the containing `AttachedClients` collection.
  destroy() {
